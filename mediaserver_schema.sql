...
-- NEW FUNCTIONALITY
-- Create table for movie ratings
DROP TABLE IF EXISTS mediaserver.MovieRatings CASCADE;

CREATE TABLE mediaserver.MovieRatings(
    movie_id INTEGER REFERENCES mediaserver.Movie(movie_id) PRIMARY KEY,
    imdb_score DECIMAL,
    imdb_votes INTEGER,
    rt_score DECIMAL,
    top100 boolean DEFAULT FALSE,
    top250 boolean DEFAULT FALSE,
    last_updated DATE
);

-- Stored function for adding to new movieRatings table
CREATE OR REPLACE FUNCTION mediaserver.addMovieRatings(
    _movie_id INTEGER,
    _imdb_score NUMERIC,
    _imdb_votes INTEGER,
    _rt_score NUMERIC,
...
BEGIN
    INSERT INTO mediaserver.MovieRatings
        VALUES(
			_movie_id,
            _imdb_score,
            _imdb_votes,
        ...
        ON CONFLICT (movie_id) DO UPDATE 
			SET imdb_score = _imdb_score,
				imdb_votes = _imdb_votes,
				rt_score = _rt_score,
				top100 = _top100,
				top250 = _top250,
